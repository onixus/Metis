...
Пустой URL отключает адаптер. Запрос публикации ADR без базы знаний отклоняется API; отсутствие адаптера не является успешным выполнением внешней команды. Синхронизацию трекера и проверку продлений worker выполняет с интервалами `METIS_DELIVERY_SYNC_INTERVAL` (по умолчанию `1h`) и `METIS_RENEWAL_INTERVAL` (`24h`).
